...
static void gen_uniforms(julia_set_t *js);
...
void update_julia_set_C(julia_set_t *js, float a, float b)
{
	js->C.value_f2[0] = a;
	js->C.value_f2[1] = b;
	set_uniform(js->program, &js->C);
}

void update_julia_set_Scale(julia_set_t *js, float a, float b)
{
...
void update_julia_set_Center(julia_set_t *js, float a, float b)
{
...
void update_julia_set_Size(julia_set_t *js, int a, int b)
{
...
void update_julia_set_MaxIteration(julia_set_t *js, int a)
{
...
{
	js->C.name = "C";
	js->C.type = GL_FLOAT_VEC2;
	js->C.value_f2[0] = -0.4;
	js->C.value_f2[1] = 0.6;
	create_uniform(js->program, &js->C);
			...
			case GLFW_KEY_W:
				{
					...
					update_julia_set_Scale(js, v1, v2);
				}
				break;
			...
			case GLFW_KEY_V:
				{
					...
					update_julia_set_Center(js, x, y);
				}
				break;
		}
	}
